...
```{r}
data <- read.csv("/Users/jianingcai/Documents/GitHub/dspg23census/project_web/economy_compilation.csv")
...
for (i in 1:nrow(data)) {
  #--------------------------income----------------------------
  if (any(grepl("income", data[i,3]))) {
    if(any(grepl("table download", data[i,4]))){
        new_row <- c("table download", "income",46)
        df_stack2 <- rbind(df_stack2, new_row)
        }
    if(any(grepl("table", data[i,4]))){
        new_row <- c("table", "income",46)
        df_stack2 <- rbind(df_stack2, new_row)
        }
    if(any(grepl("report", data[i,4]))){
        new_row <- c("report", "income",46)
        df_stack2 <- rbind(df_stack2, new_row)
        }
    if(any(grepl("map", data[i,4]))){
        new_row <- c("map", "income",46)
        df_stack2 <- rbind(df_stack2, new_row)
        }
    if(any(grepl("visualization", data[i,4]))){
        new_row <- c("data visualization", "income",46)
        df_stack2 <- rbind(df_stack2, new_row)
        }
  }
  #---------------------------employment----------------------------
  if (any(grepl("employment", data[i,3]))) {
    if(any(grepl("table download", data[i,4]))){
  ...
  #---------------------------labor force----------------------------
  if (any(grepl("labor force", data[i,3]))) {
    if(any(grepl("table download", data[i,4]))){
