...
sub_cat_counts <- function(state) {
  if(state=="All Sample States") {
    sub_cats <- ggplot(data=subset(housing_data, !is.na(Sub.categories)), aes(x=Sub.categories)) + geom_bar(fill="blue") + labs(x="Sub-Category", y="Counts", title="Types of Sub-Categories") + theme(axis.text.x = element_text(angle = 25)) + scale_x_discrete(drop = TRUE)
    ggplotly(sub_cats)
  }
  else { #fix
    ...
    sub_cats <- ggplot(state_df, aes(x=Sub.categories)) + geom_bar(fill="blue") + labs(x="Sub-Category", y="Counts", title="Types of Sub-Categories") + theme(axis.text.x = element_text(angle = 25)) + scale_x_discrete(drop = TRUE)
    ggplotly(sub_cats) 
  }
}
